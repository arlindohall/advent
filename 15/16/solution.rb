...
class Ticker
  def aunt_sue
    ...
    text.strip.lines.map do |line|
      item, number = line.strip.split(': ')
      [item, number.to_i]
    end.each do |item, number|
      sue.send(:"#{item}=", number)
    end
    sue
  end
end

FactSheet = Struct.new(:text)
class FactSheet
  def aunt_sues
    text.strip.lines.map(&:strip).map do |line|
      line = line.split('Sue ').last
      number, rest = line.match(/(\d+): (.*)/).captures
      sue = AuntSue.new(number.to_i)

      rest.split(', ').each do |property|
        name, value = property.split(': ')
        sue.send(:"#{name}=", value.to_i)
      end
  ...
  def find aunt_sue
    aunt_sues.filter { |sue| sue.match?(aunt_sue) }
  end

  def find_pt2 aunt_sue
    aunt_sues.filter { |sue| sue.match_pt2?(aunt_sue) }
  end
end

AuntSue = Struct.new(:number, :children, :cats, :samoyeds, :pomeranians, :akitas, :vizslas, :goldfish, :trees, :cars, :perfumes)
class AuntSue
  def match? aunt_sue
    for property in [:children, :cats, :samoyeds, :pomeranians, :akitas, :vizslas, :goldfish, :trees, :cars, :perfumes]
      return false if send(property) && send(property) != aunt_sue.send(property)
    end
    true
  end

  def match_pt2? real_aunt_sue
    for property in [:children, :samoyeds, :akitas, :vizslas, :cars, :perfumes]
      return false if send(property) && send(property) != real_aunt_sue.send(property)
    end

    # real aunt sue from ticker has more than N cats
    # check if this aunt sue has more than N cats
    for property in [:cats, :trees]
      return false if send(property) && send(property) <= real_aunt_sue.send(property)
    end

    # real aunt sue from ticker has fewer than N pomeranians
    # check if this aunt sue has more than N pomeranians
    for property in [:pomeranians, :goldfish]
      return false if send(property) && send(property) >= real_aunt_sue.send(property)
    end
