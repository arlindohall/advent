...
class Instructions
  ...
  def visited
    each_move do
      visit_current
      travel_move
      # visiting is idempotent so we can catch both the first
      # and last location by calling twice
      visit_current
    end
  ...
  def visit_current
    @visited.add(@santa.location)
  end

  def visited_with_robot
    @mover = :santa
    each_move do
      visit_with_robot
      travel_with_robot
  ...
  def visit_with_robot
    @visited.add(@robot.location)
    @visited.add(@santa.location)
  end

  def travel_with_robot
    if @mover == :santa
      @santa = @santa.move(@move)
    elsif @mover == :robot
      @robot = @robot.move(@move)
    else
      raise "Unknown mover: #{@mover}"
    end
  end
