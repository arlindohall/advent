...
Expansion = Struct.new(:characters, :times, :part)
...
class Message
  ...
  def fully_expand
    expanded = self
    while expanded.contains_groups?
  ...
  def contains_groups?
    parse.any? { |part| part.is_a?(Expansion) }
  end
  ...
  def read_expansion
    ...
    part = read_chars(characters)
    Expansion.new(characters, times, part)
  end
...
@example2 = <<-END.strip.lines.map(&:strip)
  (3x3)XYZ
  X(8x2)(3x3)ABCY
