...
class Room
  ...
  def decypher
    name.chars.map do |char|
      ...
      when '-'
        ...
        [((char.byte - BASE + rotate - 1) % NUM_LETTERS) + BASE].pack('U')
      end
    end.join
  end

  def is_north
    decypher.include?('north')
  end

  def sector_id
    @sector_id ||= parse[2].to_i
  end

  def rotate
    sector_id % NUM_LETTERS + 1
  end
  ...
  def top_five
    @top_five ||= begin
      ...
      while top.size < 5
        next_group = counts[keys.pop].sort
        top = (top + next_group).take(5)
      end
...
def part1
  @rooms.filter(&:valid?).map(&:sector_id).sum
end

def part2
  @rooms.filter(&:valid?).filter(&:is_north)
end
...
).strip.lines.map(&:strip).map{ |l| Room.new(l) }