...
Bot = Struct.new(:number, :high_dest, :low_dest, :high, :low)
Input = Struct.new(:value, :bot)
Output = Struct.new(:value, :bot)

class Input
  def propogate
    Bot.get(bot).take(value, self)
  end
end

class Bot
  def take(input, source)
    if self.low.nil?
      self.low = input
      return
    elsif input > low
  ...
  def propogate
    high_dest.take(high, self)
    low_dest.take(low, self)
  end
...
class Output
  def take(input, source)
    self.value, self.bot = input, source
  end

  class << self
    def get(number)
      outputs[number] ||= Output.new
    end
...
class Solution
  ...
  def input(line)
    value, bot = /value (\d+) goes to bot (\d+)/.match(line).captures.map(&:to_i)
    Input.new(value, bot)
  end

  def bot(line)
    bot, low_type, low_num, high_type, high_num =
      /bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/
        .match(line)
        .captures

    Bot.get(bot.to_i).tap do |bot|
      bot.low_dest = dest(low_type, low_num)
      bot.high_dest = dest(high_type, high_num)
    end
  end

  def dest(type, number)
    case type
    when "bot"
      Bot.get(number.to_i)
    when "output"
      Output.get(number.to_i)
    end
  end
end

def part1
  Solution.new(@input).part1
end

def part2
  Solution.new(@input).propogate
  [0,1,2].map{ |i| Output.get(i) }.map(&:value).reduce(&:*)
end

@example = <<-EOF.strip.lines
