...
class PasswordList < Struct.new(:list)
  def count_valid
    list.filter(&:valid?).count
  end

  def count_new
    list.filter(&:new_valid?).count
  end

  def self.parse(list)
    new(list.split("\n").map { Password.parse(_1) })
  end
end

class Password < Struct.new(:password, :policy)
  def valid?
    policy.accepts(password)
  end

  def new_valid?
    policy.new_policy(password)
  end

  def self.parse(text)
    policy, pass = text.split(": ")
    range, letter = policy.split(" ")
    min, max = range.split("-").map(&:to_i)

    new(pass, Policy.new(letter, min, max))
  end
end

class Policy < Struct.new(:letter, :min, :max)
...
def solve
  [PasswordList.parse(@input).count_valid, PasswordList.parse(@input).count_new]
end
