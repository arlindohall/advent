...
def solve
  [
    Game.new(read_input).play,
    Game.new(read_input).play_recursive
  ]
end
...
class Game < Struct.new(:text)
  ...
  def play_recursive
    ...
    RecursiveGame.new(p1, p2, Set[])
      .play
      .then do |p1, p2|
        raise "Should only have one winner (#{p1}, #{p1})" unless p1 == 0 || p2 == 0
        p1 + p2
      end
  end
  ...
  def round
    debug(player1: players.first.cards, player2: players.second.cards)
  ...
  class RecursiveGame < Struct.new(:p1_cards, :p2_cards, :games, :level)
    ...
    def round
      debug(p1_cards:, p2_cards:, game_count: games.size, level:)
      games << self
      c1 = p1_cards.shift
      c2 = p2_cards.shift
    ...
    def subgame(c1, c2)
      ...
      return p2_sub_cards.zero? ||
        ended_early?(p1_sub_cards, p2_sub_cards)
    end
  ...
  class Player < Struct.new(:block)
    ...
    def <<(card)
      debug(adding_card: card, to_deck: name, having: cards)
      cards << card
      self
    end
