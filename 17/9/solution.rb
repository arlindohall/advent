...
Garbage = Struct.new(:start, :stop, :unescaped)
Group = Struct.new(:start, :stop, :children, :level)

class Group
  def score
    level + children.map(&:score).sum
  end

  def groups
    1 + children.map(&:groups).sum
  end

  def garbage
    children.map(&:garbage).sum
  end
end
...
class Stream
  ...
  def solve
    [stream.first.score, stream.first.garbage]
  end
  ...
  def read_token
    ...
    when '{'
      ...
      raise "Unexpected character: #{context}, index=#{@index}, tokens=#{@stream.count}"
    end
  end
  ...
  def garbage
    ...
    Garbage.new(start, @index, unescaped)
  end

  def group(level = 1)
    start, children = @index, []
    advance
    return empty_group(level) if current == '}'

    read_children(children, level+1)
    raise "Unclosed bracket #{context}, index=#{@index}, tokens=#{@stream.count}" if !consume('}')

    Group.new(start, @index, children, level)
  end

  def empty_group(level)
    advance
    Group.new(@index-2, @index, [], level)
  end

  def read_children(children, level)
    loop do
      children << read_child(level)
      return if !consume(',')
    end
  end

  def read_child(level)
    case current
    when '<'
      return garbage
    when '{'
      ...
      raise "Only expect garbage or group inside group #{context}, index=#{@index}, tokens=#{@stream.count}"
    end
  end
  ...
  def current
    raise "Out of bounds: index=#{@index}, tokens=#{@stream.count}" if @index > @text.length
    @text[@index]
  end

  def context
    @text[
      ([@index - 10, 0].max)..(@index + 10)
    ]
  end
...
@example = <<-STREAMS.strip.lines.map(&:strip)
{}
{{{}}}
{{},{}}
{{{},{},{{}}}}
...
@example2 = <<-STREAMS.strip.lines.map(&:strip)
{}
{{{}}}
{{},{}}
{{{},{},{{}}}}
