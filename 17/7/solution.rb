...
Node = Struct.new(:name, :weight, :children)
...
class Node
  class << self
    def parse(line)
      name, weight, arrow, *children = line.split

      if arrow.nil?
        return Node.new(name, weight.chars.drop(1).join.to_i, [])
      end

      Node.new(name, weight.chars.drop(1).join.to_i, children.map { |c| c.split(',').first })
    end
  end
end
...
class Tower
  ...
  def correct_weight
    desired_total_weight = wrong_node.children
      .map(&:total_weight)
      .group_by(&:itself)
      .values
      .max_by(&:size)
    ...
    mistake = wrong_node.children.find { |c| c.total_weight != desired_total_weight }

    mistake.weight + (desired_total_weight - mistake.total_weight)
  end

  def wrong_node
    node = root
    while node.children.any?(&:unbalanced?)
      node = node.children.find(&:unbalanced?)
    end
    node
  end
  ...
  def children
    @children ||= graph.flat_map(&:children).uniq
  end
  ...
  def add_node(node)
    get_node(node.name).weight = node.weight
    node.children.each do |child|
      get_node(node.name).children << get_node(child)
    end
  end

  def get_node(name)
    @graph[name] ||= Node.new(name, 0, [])
  end
