...
class Spreadsheet
  ...
  def compile
    @data = @text.strip
      ...
      .map(&:strip)
      .map(&:split)
      .map{|l| Row.new(l.map(&:to_i))}
  end

  def checksum
    @data.map(&:checksum).sum
  end

  def evenly_divisible
    @data.map(&:divisible_difference).sum
  end
end
...
class Row
  ...
  def divisible_difference
    mutually_divisible_pair.sort.reverse.reduce(&:/)
  end

  def mutually_divisible_pair
    pairs.filter{|pair| mutually_divisible?(pair)}.first
  end
  ...
  def mutually_divisible?(pair)
    pair.first % pair.last == 0 ||
