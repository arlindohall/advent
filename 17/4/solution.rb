...
class PassphraseList
  def initialize(text)
    @passphrases = text
      ...
      .map(&:strip)
      .map(&:split)
      .map{|line| Passphrase.new(line)}
  end

  def valid
    @passphrases.filter(&:valid?)
  end

  def added_security
    @passphrases.filter(&:added_security?)
  end
end
...
class Passphrase
  ...
  def added_security?
    sorted_passphrase.valid?
  end

  def sorted_passphrase
    Passphrase.new(sorted_words)
  end
