...
MachineState = Struct.new(:registers)
Instruction = Struct.new(:destination, :direction, :amount, :source, :op, :compare)

class MachineState
  def apply(instruction)
    MachineState.new(
      registers.merge({
        instruction.destination => test(instruction, get(instruction.destination)) ?
          modify(instruction, get(instruction.destination)) :
          get(instruction.destination)
      })
    )
  end

  def test(instruction, value)
    get(instruction.source).send(instruction.op.to_sym, instruction.compare)
  end

  def modify(instruction, value)
    if instruction.direction == 'dec'
...
class Machine
  ...
  def parse(line)
    destination, direction, amount, _if, source, op, compare =
      line.split(' ')
    Instruction[destination, direction, amount.to_i, source, op, compare.to_i]
  end
  ...
  def execute
    ...
    @program.each do |instruction|
      @machine_state = @machine_state.apply(instruction)
      @max = [@max, @machine_state.registers.values.max].max
    end

    [@machine_state.registers.values.max, @max]
  end
end
